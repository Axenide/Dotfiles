...
class Notch(Window):
    ...
    def colorpicker(self, button, event):
        if event.button == 1:
            GLib.spawn_command_line_async(f"bash {data.HOME}/.config/fabric/scripts/hyprpicker-hex.sh")
        elif event.button == 2:
            GLib.spawn_command_line_async(f"bash {data.HOME}/.config/fabric/scripts/hyprpicker-hsv.sh")
        elif event.button == 3:
            GLib.spawn_command_line_async(f"bash {data.HOME}/.config/fabric/scripts/hyprpicker-rgb.sh")
